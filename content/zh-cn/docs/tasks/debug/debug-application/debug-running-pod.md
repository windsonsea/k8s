...
-->
## 调试配置文件   {#debugging-profiles}

使用 `kubectl debug` 通过调试 Pod 来调试节点、通过临时容器来调试 Pod 或者调试复制的 Pod 时，
你可以使用 `--profile` 标志为其应用调试配置文件。通过应用配置文件，可以设置特定的属性（如
...
-->
如果你不指定 `--profile`，`legacy` 配置文件被默认使用，但此配置文件计划在不久的将来弃用。
因此建议使用 `general` 等其他配置文件。
{{< /note >}}
...
-->
然后，使用临时容器调试 Pod。
如果临时容器需要具有特权，你可以使用 `sysadmin` 配置文件：
...
-->
这意味着通过应用 `sysadmin` 配置文件，容器进程被授予了作为特权容器的全部权能。
