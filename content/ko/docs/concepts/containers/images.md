...
레지스트리 호스트 이름을 지정하지 않으면, 쿠버네티스는
[도커 퍼블릭 레지스트리](https://hub.docker.com/)를 의미한다고 가정한다.
이 동작은 [컨테이너 런타임](/ko/docs/setup/production-environment/container-runtimes/)
설정에서 기본 이미지 레지스트리를 설정하여 변경할 수 있다.

이미지 이름 부분 다음에 _tag_ 또는 _digest_ 를 추가할 수 있다
(`docker` 또는 `podman` 과 같은 명령을 사용할 때와 동일한 방식으로).
태그를 사용하면 동일한 시리즈 이미지의 다른 버전을 식별할 수 있다.
다이제스트는 이미지의 특정 버전에 대한 고유 식별자이다. 다이제스트는
이미지 콘텐츠의 해시이며 변경 불가능하다. 태그는 다른 이미지를
가리키도록 변경될 수 있지만, 다이제스트는 고정된다.
...
[OCI Distribution Specification](https://github.com/opencontainers/distribution-spec/blob/master/spec.md#workflow-categories)에서
확인할 수 있다.
...
[OCI Image Specification](https://github.com/opencontainers/image-spec/blob/master/descriptor.md#digests)에서
확인할 수 있다.
...
생성한다. 이전 버전과의 호환성을 위해, 접미사가 있는 오래된 이미지를 생성한다.
예를 들어, `pause`라는 이미지는 지원되는 모든 아키텍처에 대한 매니페스트를 포함하는
다중 아키텍처 이미지이고, `pause-amd64`는 이전 구성이나 접미사가 포함된 이미지 이름을
하드 코딩한 YAML 파일과 호환되는 버전이다.

## 프라이빗 레지스트리 사용

프라이빗 레지스트리는 이미지를 검색하거나 풀하기 위해 인증을 요구할 수 있다.
...
가장 강력하고 다재다능한 방법이지만, 활성화하기 위해 kubelet 수준의 구성이 필요하다.

이 기법은 프라이빗 레지스트리에 호스팅된 컨테이너 이미지가 필요한
{{< glossary_tooltip term_id="static-pod" text="스태틱 파드" >}}를
실행할 때 특히 유용하다.
...
유일한 제한 사항은 glob 패턴(`*`)이 각 서브도메인에 대해 점(`.`)을 포함해야 한다는 것이다. 
매치되는 서브도메인의 수는 glob 패턴(`*.`)의 수와 같아야 한다. 예를 들어,

- `*.kubernetes.io`는 `kubernetes.io`와는 매치되지 *않지만*, `abc.kubernetes.io`와는 매치된다.
- `*.*.kubernetes.io`는 `abc.kubernetes.io`와는 매치되지 *않지만*, `abc.def.kubernetes.io`와는 매치된다.
...
루트 URL(`*.my-registry.example`)은 다음 문법을 사용하여 매치된다.
...
미리 내려받은 이미지에 대한 접근은 [이미지 풀 자격증명 검증 보장](#ensureimagepullcredentialverification)에
따라 인가될 수 있다.
{{< /note >}}
...
쿠버네티스는 자격증명이 필요한 모든 이미지에 대해 이미지 자격증명을 검증한다.
이미지가 이미 노드에 있더라도 마찬가지이다. 이 검증은 제공된 자격증명으로
성공적으로 풀되지 않은 파드 요청의 이미지가 레지스트리에서 이미지를
다시 풀해야 함을 보장한다.
또한, 이전에 성공적인 이미지 풀을 수행한 것과 동일한 자격증명을 재사용하는
이미지 풀은 레지스트리에서 다시 풀할 필요 없이 레지스트리에 접근하지 않고
로컬에서 검증된다(이미지가 로컬에서 사용 가능한 경우).
...
이 설정은 이미지가 이미 노드에 있는 경우 이미지 풀 자격증명을 언제 검증해야 하는지를 제어한다.
...
* `AlwaysVerify`: 모든 이미지는 사용되기 전에 자격증명이 검증된다.
...
미리 내려받은 것으로 간주된다. 이는 kubelet이 풀된 이미지에 대한 기록이 없기 때문이다.
kubelet은 이미지가 처음 풀될 때부터 이미지 풀 기록을 만들 수 있다.
...
그러나, [ServiceAccount](/docs/tasks/configure-pod-container/configure-service-account/)
리소스에 `imagePullSecrets` 섹션을 설정하여 이 과정을 자동화할 수 있다.
...
쿠버네티스 버전 1.26부터 레거시 메커니즘이 제거되었으므로, 다음 중 하나를 수행해야 한다.
...
* [프라이빗 레지스트리에서 이미지 받아오기](/ko/docs/tasks/configure-pod-container/pull-image-private-registry)에 대해 자세히 알아보기.
