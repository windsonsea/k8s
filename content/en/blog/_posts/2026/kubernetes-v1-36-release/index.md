...
Now, users can use `kubectl describe pod` to determine if a container's crash loop is 
...
As of v1.35, the scheduler could perform [gang scheduling](https://kubernetes.io/docs/concepts/scheduling-eviction/gang-scheduling/) by ensuring 
a minimum number of pods are ready before any are bound, 
...
and DRA Native Resources to simplify how drivers define hardware classes.
...
The `externalIPs` field in Service `spec` is deprecated, which means you no longer have a way to route arbitrary externalIPs 
to your Services. This field has been a known security headache for years, enabling man-in-the-middle attacks on your 
cluster traffic, as documented in [CVE-2020-8554](https://github.com/kubernetes/kubernetes/issues/97076). 
...
You can also easily install v1.34 using kubeadm.
